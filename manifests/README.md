...
- Chose : Deploy to Azure Kubernetes Service
  - Cluster: aksdemo3
...
## Notes (for me):
When you create one of pre-build pipeline, 3 thinks are created:

1. generate pipeline code
2. create service connection:
    - K8S
    - Docker registry: and creates an ap with acr role on it
    - Github
3. create environment (k8s environment) QUESTION : what is the advantage to this ? ==> to distinguish between clusters (those for Dev, those for QA and those fot PROD) BUT it works only for release pipelines. Notice if you create an new env, then a new service connection will be created



- Notice dependencies between stages. 
